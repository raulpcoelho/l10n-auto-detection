...
parser.add_argument("--lang", "-l", help="App language", default="pt-br")
parser.add_argument(
    "--custom-ignore", "-i", help="Custom ignore file path", default=None
)
...
acceptable_words = []
lang = args.lang
with open(f"dictionaries/{lang}.txt", "r", encoding="utf-8") as file:
    acceptable_words = [line.strip() for line in file]

ignore_words = []
with open(f"dictionaries/ignore_{lang}.txt", "r", encoding="utf-8") as file:
    ignore_words = [line.strip() for line in file]

if args.custom_ignore:
    with open(args.custom_ignore, "r", encoding="utf-8") as file:
        ignore_words += [line.strip() for line in file]
